...
function loadAnimeList() {
    ...
    if (currentPage > 1 && lastVisible) {
        // You would need a more complex way to get the previous 'lastVisible' in a real scenario
        // For simplicity here, we rely on the `timestamp` order to approximate.
        // For a reliable 'previous page' button with startAfter, you need to store the startAt cursor of the *current* page.
        // We skip the startAfter logic for previous page here for simplicity and rely on the next page logic below.
    }
    
    // To implement `goToPreviousPage` reliably, you need to save the array of start cursors (lastVisible)
    // For now, we only use `startAfter` for `goToNextPage`.
    // Since this is a simple example, we will only apply pagination logic if we go to the next page
    if (currentPage > 1 && lastVisible) {
         // This is a simplified approach, highly efficient for next page only.
...
function goToPreviousPage() {
    if (currentPage > 1) {
        currentPage--;
        lastVisible = null; // In the simplified code, resetting lastVisible is the easiest way.
        loadAnimeList();
    }
}
...
function goToNextPage() {
    currentPage++;
    // lastVisible is set in loadAnimeList
    loadAnimeList();
}
...
function renderAdminList() {
    ...
    listEl.innerHTML = generateSkeletonCards(5).replace(/card/g, 'admin-list-item'); 

    // This query will also need an index: seriesId: asc, season: asc
...
function loadEpisodeList(seriesId, activeDocId) {
    ...
    // This query will also need an index: seriesId: asc, season: asc, episode: asc
    db.collection("animes")
